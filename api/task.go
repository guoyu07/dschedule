...
func (s *HTTPServer) TaskEndpoint(resp http.ResponseWriter, req *http.Request) (interface{}, error) {
	...
	case "DELETE":
		...
		taskIdStr := strings.TrimPrefix(req.URL.Path, "/task/")
		if taskIdStr != "" {
			taskId, err := strconv.Atoi(taskIdStr)
			if err != nil {
				return nil, fmt.Errorf("TaskId needed int: %s", err)
			}
...
func (s *HTTPServer) deleteTask(resp http.ResponseWriter, req *http.Request) (interface{}, error) {
	//todo
}
